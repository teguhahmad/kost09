...
const Notifications = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // First, get the backoffice users
      ...
      // Then, get the corresponding user emails from auth.users
      ...
      // Combine the data
      const transformedUsers = backofficeUsers?.map(backofficeUser => ({
        user_id: backofficeUser.user_id,
        role: backofficeUser.role,
        email: userData?.users.find(user => user.id === backofficeUser.user_id)?.email
      })) || [];
