...
const Notifications = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Then, get user profiles from auth.users instead of public.users
      const { data: profiles, error: profilesError } = await supabase
        .from('auth.users')
        .select('id, email')
        .in('id', backofficeUsers?.map(user => user.user_id) || []);
      ...
      const transformedUsers = backofficeUsers?.map(user => ({
        ...
        role: user.role,
        email: profiles?.find(profile => profile.id === user.user_id)?.email
      })) || [];
