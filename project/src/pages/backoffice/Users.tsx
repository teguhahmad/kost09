...
const BackofficeUsers: React.FC = () => {
  ...
  const loadUsers = async () => {
    try {
      ...
      // Get user profiles for each backoffice user
      const usersWithProfiles = await Promise.all(
        backofficeUsers.map(async (user) => {
          const { data: { user: profile }, error: profileError } = await supabase.auth.admin.getUserById(user.user_id);
